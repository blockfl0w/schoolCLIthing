            ...
            while True:
                ...
                if user["password"] != password:
                    ...
                    print(
                        "\033[32m"
                        + user["username"]
                        + ", you have been successfully logged in!\033[0m"
                    )
                    global role
                    role = user["role"]
                    global username
...
def reportAdd():
    name = input(
        "Enter a name for the report (leave blank to get an auto generated one): "
    )

    # if the users logged in ask if they want to submit anonymously
    if username != "":
        anonymous = input(
            "Would you like to submit this report anonymously? (y/N): "
        ).lower()
    else:
        anonymous = "n"
    ...
    report["name"] = name
    report["reviewer"] = (
        "Anonymous" if anonymous == "y" or anonymous == "yes" else username
    )
    ...
    if len(args) == 0 and role == "user":
        ...
        with open("./data/reports.json", "r") as file:
            ...
            filteredReports = list(
                filter(lambda report: report["reviewer"] == username, reports)
            )
            for i, report in enumerate(filteredReports):
                ...
                if len(filteredReports) - (i + 1) != 0:
                    prompt = input(
                        f"Would you like to view the next report ({len(filteredReports) - (i + 1)} left)? (Y/n)"
                    ).lower()
        ...
        with open("./data/reports.json", "r") as file:
            ...
            for i, report in enumerate(reports):
                ...
                if len(reports) - (i + 1) != 0:
                    prompt = input(
                        f"Would you like to view the next report ({len(reports) - (i + 1)} left)? (Y/n)"
                    ).lower()
                    if prompt == "n" or prompt == "no":
                        break
...
def deleteReport(reports, report, account=False):
    conformation = input(
        f"Are you sure you want to delete this {'account' if account else 'report'}? (Y/n)"
    ).lower()
    ...
    def do_account(self, line):
        ...
        if len(args) == 0:
            ...
            with open("./data/users.json", "r") as file:
                ...
                user = list(
                    filter(lambda report: report["username"] == username, users)
                )
